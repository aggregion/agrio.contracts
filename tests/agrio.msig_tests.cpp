...
public:
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(agrio), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(agrio), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, agrio_msig_tester ) try {
   ...
   set_authority(N(agrio), "active", authority(1,
      ...
      { { N(agrio), "active" } }, { get_private_key( N(agrio), "active" ) });
   ...
   create_currency( N(agrio.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   ...
   produce_blocks();
   create_account_with_resources( N(alice1111111), N(agrio), core_sym::from_string("1.0000"), false );
   create_account_with_resources( N(bob111111111), N(agrio), core_sym::from_string("0.4500"), false );
   create_account_with_resources( N(carol1111111), N(agrio), core_sym::from_string("1.0000"), false );
   ...
   vector<permission_level> action_perm = {{N(agrio), config::active_name}};
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(agrio), core_sym::from_string("1.0000"), false ),
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, agrio_msig_tester ) try {
   ...
   create_currency( N(agrio.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   issue(config::system_account_name, core_sym::from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance( "agrio" ) );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_invalidate, agrio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_invalidate_approve, agrio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( approve_execute_old, agrio_msig_tester ) try {
   set_code( N(agrio.msig), contracts::util::msig_wasm_old() );
   set_abi( N(agrio.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();

   //propose with old version of agrio.msig
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   push_action( N(alice), N(propose), mvo()
                  ("proposer",      "alice")
                  ("proposal_name", "first")
...
BOOST_FIXTURE_TEST_CASE( approve_unapprove_old, agrio_msig_tester ) try {
   set_code( N(agrio.msig), contracts::util::msig_wasm_old() );
   set_abi( N(agrio.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();
...
BOOST_FIXTURE_TEST_CASE( approve_by_two_old, agrio_msig_tester ) try {
   set_code( N(agrio.msig), contracts::util::msig_wasm_old() );
   set_abi( N(agrio.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();
...
BOOST_FIXTURE_TEST_CASE( approve_with_hash, agrio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(approve), mvo()
                            ...
                            agrio::chain::crypto_api_exception,
...
BOOST_FIXTURE_TEST_CASE( switch_proposal_and_fail_approve_with_hash, agrio_msig_tester ) try {
