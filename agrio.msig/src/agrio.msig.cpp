#include <agrio.msig/agrio.msig.hpp>
#include <agriolib/action.hpp>
#include <agriolib/permission.hpp>
                        ...
                        ignore<transaction> trx )
{
   ...
   agrio_assert( _trx_header.expiration >= agrio::time_point_sec(current_time_point()), "transaction expired" );
   //agrio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, _proposer.value );
   agrio_assert( proptable.find( _proposal_name.value ) == proptable.end(), "proposal with the same name exists" );
   ...
   agrio_assert( res > 0, "transaction authorization failed" );
                        ...
                        const agrio::binary_extension<agrio::checksum256>& proposal_hash )
{
   ...
   if ( apps_it != apptable.end() ) {
      ...
      agrio_assert( itr != apps_it->requested_approvals.end(), "approval is not on the list of requested approvals" );
      ...
      agrio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( name proposer, name proposal_name, permission_level level ) {
   ...
   if ( apps_it != apptable.end() ) {
      ...
      agrio_assert( itr != apps_it->provided_approvals.end(), "no approval previously granted" );
      apptable.modify( apps_it, proposer, [&]( auto& a ) {
      ...
      agrio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
      old_apptable.modify( apps, proposer, [&]( auto& a ) {
...
void multisig::cancel( name proposer, name proposal_name, name canceler ) {
   ...
   if( canceler != proposer ) {
      agrio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < agrio::time_point_sec(current_time_point()), "cannot cancel until expiration" );
   }
   ...
   if ( apps_it != apptable.end() ) {
      ...
      agrio_assert( apps_it != old_apptable.end(), "proposal not found" );
...
void multisig::exec( name proposer, name proposal_name, name executer ) {
   ...
   ds >> trx_header;
   agrio_assert( trx_header.expiration >= agrio::time_point_sec(current_time_point()), "transaction expired" );
...
AGRIO_DISPATCH( agrio::multisig, (propose)(approve)(unapprove)(cancel)(exec)(invalidate) )
